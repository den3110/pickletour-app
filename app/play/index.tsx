  ...
  return (
    ...
    >
      <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
        <View style={{ width: 38, height: 38, borderRadius: 19, backgroundColor: C.border, overflow: "hidden", alignItems: "center", justifyContent: "center" }}>
          {it.host?.avatar ? (
            <Image source={{ uri: it.host.avatar }} style={{ width: "100%", height: "100%" }} />
          ) : (
            <Text style={{ fontWeight: "700", color: C.sub }}>{(it.host?.name || "?").charAt(0)}</Text>
          )}
        </View>
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ fontSize: 12, color: C.muted }}>đăng kèo</Text>
        </View>
      ...
      <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
        ...
        <Text style={{ fontSize: 13.5, fontWeight: "700", color: C.text }}>{formatPlayTime(it.playAt)}</Text>
      </View>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
        <Ionicons name="location-outline" size={15} color={C.sub} />
        <Text numberOfLines={1} style={{ fontSize: 13, color: C.sub, flex: 1 }}>
      ...
      <View style={{ flexDirection: "row", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
          <Ionicons name="people-outline" size={15} color={C.sub} />
          <Text style={{ fontSize: 12.5, color: C.text2, fontWeight: "600" }}>
      ...
      {it.isHost ? (
        ...
        <Text style={{ color: C.amberText, fontWeight: "700", fontSize: 13 }}>Đã xin — chờ duyệt</Text>
...
export default function PlayScreen() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }} edges={["top"]}>
      <View style={{ paddingHorizontal: 14, paddingBottom: 10, backgroundColor: C.card, borderBottomWidth: 1, borderBottomColor: C.border }}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
          <TouchableOpacity onPress={() => router.back()} hitSlop={8}>
            <Ionicons name="chevron-back" size={26} color={C.text} />
          </TouchableOpacity>
          <Text style={{ fontSize: 20, fontWeight: "900", flex: 1, color: C.text }}>🏓 Tìm bạn đánh</Text>
        </View>
        {/* Tabs */}
          ...
          {[{ k: false, label: "Khám phá" }, { k: true, label: "Kèo của tôi" }].map((t) => {
            ...
            return (
              <TouchableOpacity
                ...
                style={{ paddingHorizontal: 16, paddingVertical: 7, borderRadius: 999, backgroundColor: active ? GREEN : C.field }}
              >
        ...
        {!mine && (
          <View style={{ flexDirection: "row", gap: 8, marginTop: 10 }}>
            <View style={{ flex: 1, flexDirection: "row", alignItems: "center", backgroundColor: C.field, borderRadius: 10, paddingHorizontal: 10, height: 40 }}>
              ...
              <TextInput
                ...
                style={{ flex: 1, marginLeft: 6, fontSize: 14, color: C.text }}
              />
            </View>
            <View style={{ width: 110, flexDirection: "row", alignItems: "center", backgroundColor: C.field, borderRadius: 10, paddingHorizontal: 10, height: 40 }}>
              <TextInput
                value={skill}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={{ alignItems: "center", marginTop: 60 }}>
              ...
              <Text style={{ color: C.sub, marginTop: 8, fontWeight: "600" }}>Chưa có kèo nào phù hợp</Text>
            </View>
          }
        />
      )}
