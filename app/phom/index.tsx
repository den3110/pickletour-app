...
export default function PhomLobbyScreen() {
  ...
  useEffect(() => {
    if (!socket) return;
    socket.emit("phom:lobby:subscribe");
    const onUpdate = () => refetch();
    socket.on("phom:lobby:updated", onUpdate);
    return () => {
      socket.off("phom:lobby:updated", onUpdate);
      socket.emit("phom:lobby:unsubscribe");
    };
  }, [socket, refetch]);
  ...
  return (
    <SafeAreaView style={styles.container} edges={["bottom"]}>
      ...
      <View style={styles.notice}>
        <Ionicons name="information-circle-outline" size={14} color={C.amberText} />
...
const mk_styles = (C: ThemeTokens) => StyleSheet.create({
  ...
  notice: {
    ...
    borderBottomWidth: 1,
    borderBottomColor: C.dark ? "rgba(245,158,11,0.35)" : "#FDE68A",
    alignItems: "flex-start",
  },
  noticeText: { flex: 1, fontSize: 11, color: C.amberText, lineHeight: 15 },
  card: {
    backgroundColor: C.card,
    borderRadius: 12,
    padding: 12,
